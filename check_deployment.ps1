...
if ($azInstalled) {
    ...
    az webapp show --name demoecomerce --resource-group ecomerce_test --query "{name:name, state:state, defaultHostName:defaultHostName}" -o table 2>$null
    
    Write-Host ""
    Write-Host "📋 Últimos deployments:" -ForegroundColor Yellow
    az webapp deployment list --name demoecomerce --resource-group ecomerce_test --query "[0:3].{status:status, active:active, start:start_time, author:author}" -o table 2>$null
    
    Write-Host ""
    Write-Host "📊 Para ver logs en tiempo real, ejecuta:" -ForegroundColor Cyan
    Write-Host "   az webapp log tail --name demoecomerce --resource-group ecomerce_test" -ForegroundColor White
} else {
...
Write-Host "   App URL: https://demoecomerce.azurewebsites.net" -ForegroundColor White
Write-Host ""

# Intentar hacer ping a la aplicación
Write-Host "🔍 Verificando si la app responde..." -ForegroundColor Yellow
try {
    $response = Invoke-WebRequest -Uri "https://demoecomerce.azurewebsites.net" -Method Head -TimeoutSec 10 -ErrorAction Stop
