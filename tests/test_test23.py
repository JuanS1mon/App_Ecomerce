...
def test_create_test23(codigo_value):
    response = client.post("/test23/", json={
        "codigo": codigo_value,
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_gets_test23_all(codigo_value):
    response = client.get("/test23/")
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_get_test23_by_id(codigo_value):
    # Asegurarnos de que el registro existe
    response = client.get(f"/test23/id/{codigo_value}")
    if response.status_code == 404:
        # Crear el registro si no existe
        client.post("/test23/", json={
            "codigo": codigo_value,
            "descripcion": f'Valor de prueba {codigo_value}',
        })
        response = client.get(f"/test23/id/{codigo_value}")
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_update_test23(codigo_value):
    response = client.put(f"/test23/id/{codigo_value}", json={
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_delete_test23(codigo_value):
    response = client.delete(f"/test23/id/{codigo_value}")
    assert response.status_code == 200
    data = response.json()
    assert data["codigo"] == codigo_value
...
def test_create_test23_invalid():
    response = client.post("/test23/", json={
        "descripcion": None,
    })
