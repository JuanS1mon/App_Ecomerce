...
def test_create_test3(codigo_value):
    response = client.post("/test3/", json={
        "codigo": codigo_value,
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_gets_test3_all(codigo_value):
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_get_test3_by_id(codigo_value):
    # Asegurarnos de que el registro existe
    response = client.get(f"/test3/id/{codigo_value}")
    if response.status_code == 404:
        # Crear el registro si no existe
        client.post("/test3/", json={
            "codigo": codigo_value,
            "fecha": f'Valor de prueba {codigo_value}',
            "nombre": f'Valor de prueba {codigo_value}',
        })
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_update_test3(codigo_value):
    response = client.put(f"/test3/id/{codigo_value}", json={
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_delete_test3(codigo_value):
    response = client.delete(f"/test3/id/{codigo_value}")
    assert response.status_code == 200
    data = response.json()
    assert data["codigo"] == codigo_value
