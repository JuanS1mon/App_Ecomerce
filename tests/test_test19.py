...
def test_create_test19(codigo_value):
    response = client.post("/test19/", json={
        "codigo": codigo_value,
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_gets_test19_all(codigo_value):
    response = client.get("/test19/")
    assert response.status_code == 200
    data = response.json()
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_get_test19_by_id(codigo_value):
    # Asegurarnos de que el registro existe
    response = client.get(f"/test19/id/{codigo_value}")
    if response.status_code == 404:
        # Crear el registro si no existe
        client.post("/test19/", json={
            "codigo": codigo_value,
            "descripcion": f'Valor de prueba {codigo_value}',
        })
        response = client.get(f"/test19/id/{codigo_value}")
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_update_test19(codigo_value):
    response = client.put(f"/test19/id/{codigo_value}", json={
...
@pytest.mark.parametrize("codigo_value", range(1, ITERATIONS + 1))
def test_delete_test19(codigo_value):
    response = client.delete(f"/test19/id/{codigo_value}")
    assert response.status_code == 200
    data = response.json()
    assert data["codigo"] == codigo_value
...
def test_create_test19_invalid():
    response = client.post("/test19/", json={
        "descripcion": None,
    })
