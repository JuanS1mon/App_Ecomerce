/**
 * JavaScript para la gestión de facturacion
...
async function fetchData() {
    try {
        const response = await fetch('/facturacion/');
...
function updateTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        cell1.textContent = item.nrofactura;
...
async function addItem(event) {
    ...
    const formData = {
        ...
        nrofactura: parseInt(document.getElementById('nrofactura').value) || 0
    };
...
function editItem(id) {
    fetch(`/facturacion/id/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit-nrofactura').value = data.nrofactura;
...
async function updateItem(event) {
    ...
    const updatedData = {
        nrofactura: parseInt(document.getElementById('edit-nrofactura').value) || 0
    };
    
    try {
        const response = await fetch(`/facturacion/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/facturacion/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.nrofactura).toLowerCase().includes(searchTerm);
    });
