/**
 * JavaScript para la gestión de depositos_tipos
...
async function fetchData() {
    try {
        const response = await fetch('/depositos_tipos/');
...
async function updateItem(event) {
    ...
    try {
        const response = await fetch(`/depositos_tipos/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/depositos_tipos/id/${id}`, { method: 'DELETE' });
