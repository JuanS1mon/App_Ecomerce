// Admin Dashboard JS extraído de admin.html
// Mantener un pequeño snippet inline para el boot del tema en el HTML (ya presente)

// Mapa global de tipos de actividad (conserva compatibilidad)
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  (function(){
    ...
    function current(){ return document.documentElement.classList.contains('light') ? 'light':'dark'; }
    function setTheme(t){
      document.documentElement.className = document.documentElement.className.replace(/(light|dark)/g,'').trim();
      document.documentElement.classList.add(t);
      localStorage.setItem('admin-theme', t);
      btn.setAttribute('aria-pressed', t==='dark');
      if(sun && moon){ if(t==='light'){ sun.style.opacity='1'; moon.style.opacity='0'; } else { sun.style.opacity='0'; moon.style.opacity='1'; } }
    }
    setTheme(current());
  ...
  /* Auth inicial */
  if(typeof window.checkInitialAuth === 'function'){ window.checkInitialAuth(); }
  ...
  if(chartCanvas){
    const initChart = () => {
      if(chartCanvas._initialized) return; chartCanvas._initialized = true;
      const ctx = chartCanvas.getContext('2d');
      new Chart(ctx, {
        type:'line',
        data:{ labels:['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'], datasets:[{ label:'Actividad', data:[12,19,8,15,10,3,7], backgroundColor:'rgba(99,102,241,0.2)', borderColor:'rgba(99,102,241,1)', borderWidth:2, tension:0.4, pointBackgroundColor:'rgba(99,102,241,1)', pointRadius:4 }]},
        options:{ responsive:true, maintainAspectRatio:false, scales:{ y:{ beginAtZero:true, grid:{ color:'rgba(0,0,0,0.05)'} }, x:{ grid:{ display:false } } }, plugins:{ legend:{ display:false }, tooltip:{ backgroundColor:'rgba(255,255,255,0.9)', titleColor:'#1f2937', bodyColor:'#4b5563', borderColor:'rgba(203,213,225,1)', borderWidth:1, padding:10, displayColors:false } } }
      });
    };
    const io = new IntersectionObserver(entries=>{ entries.forEach(e=>{ if(e.isIntersecting){ initChart(); io.disconnect(); } }); });
  ...
  (function(){
    ...
    const metricsData = [
      ...
      { label:'Aplicaciones', value: 4, icon:'fa-laptop-code', color:'green', link:'#' },
      { label:'Rutas', value: 4, icon:'fa-route', color:'indigo', link:'#' }
    ];
    ...
    [...grid.children].forEach((el,idx)=>{ el.classList.add('fade-in-up'); el.style.animationDelay = (idx*90)+'ms'; });
  })();

  /* Sistema avanzado de actividades */
  ...
  (function(){
    ...
    if(!btn || !list || !chk || !loadMoreBtn || !filterSelect || !scrollContainer) return;
    ...
    function formatRelative(date){
      ...
      const ranges = [ {unit:'day',secs:86400}, {unit:'hour',secs:3600}, {unit:'minute',secs:60}, {unit:'second',secs:1} ];
      if(rtf){
        for(const r of ranges){
          if(Math.abs(diffSec) >= r.secs || r.unit==='second') return rtf.format(Math.round(diffSec/r.secs), r.unit);
        }
      }
      const diff = -diffSec;
    ...
    let page = 0; const pageSize = 10; let loading=false; let hasMore=true; let currentFilter='';
    function clearList(){ list.innerHTML=''; page=0; hasMore=true; list.dataset.hasMore='true'; loadMoreBtn.disabled=false; }
    function preserveScroll(run){ const prev=scrollContainer.scrollTop; run(); scrollContainer.scrollTop=prev; }
    function buildQuery(){ const p=new URLSearchParams(); p.set('limit',pageSize); p.set('offset',page*pageSize); if(currentFilter) p.set('type',currentFilter); return '/api/activities?'+p.toString(); }
    function activityVisual(a){ const m=window.__activityTypeMap||{}; const t=(a.type||'').toLowerCase(); return m[t] || { icon:'fa-user-circle', color:'blue' }; }
    ...
    async function fetchActivities({append=false, showSkeleton=false}={}){
      ...
      spinner.classList.remove('hidden'); btn.disabled=true; btn.classList.add('opacity-60','cursor-not-allowed');
      try {
        const res = await fetch(buildQuery());
        if(!res.ok) throw new Error('HTTP '+res.status);
        const data = await res.json();
        if(!Array.isArray(data) || !data.length){ if(!append){ list.innerHTML='<li class="text-sm text-soft">Sin actividades</li>'; } hasMore=false; loadMoreBtn.disabled=true; return; }
        if(data.length < pageSize){ hasMore=false; loadMoreBtn.disabled=true; }
        const now = Date.now();
        const newMarkup = data.map((a,i)=>{ const when=a.timestamp?new Date(a.timestamp):null; const rel=formatRelative(when); const vis=activityVisual(a); const vColor=colorBg[vis.color]||colorBg.blue||'bg-blue-100 text-blue-600'; const isNew= when && (now-when.getTime())<5*60*1000; const badge=isNew?'<span class="ml-2 inline-block px-2 py-0.5 text-[10px] rounded bg-green-100 text-green-700 font-medium tracking-wide">NUEVO</span>':''; return `<li class=\"pb-3 border-b border-gray-100 last:border-0 flex items-start fade-in-up\" style=\"animation-delay:${(page*pageSize+i)*25}ms\" role=\"listitem\">\n    <div class=\"h-8 w-8 rounded-full ${vColor} flex items-center justify-center mr-3 mt-0.5\" aria-hidden=\"true\"><i class=\"fas ${vis.icon}\"></i></div>\n    <div class=\"flex-1\">\n    <p class=\"text-sm flex items-center flex-wrap\"><span class=\"font-medium\">${a.user||'Usuario'}<\/span> <span class=\"mx-1\">${a.action||''}<\/span>${badge}</p>\n    <p class=\"text-xs text-soft mt-1\"><time datetime=\"${a.timestamp||''}\">${rel}<\/time></p>\n    </div>\n    </li>`; }).join('');
        if(append){ preserveScroll(()=>list.insertAdjacentHTML('beforeend', newMarkup)); } else { list.innerHTML=newMarkup; }
        page++;
      } catch(err){ console.error(err); list.insertAdjacentHTML('afterbegin','<li class="text-xs text-red-500">Error al cargar actividades</li>'); }
      finally { spinner.classList.add('hidden'); btn.disabled=false; btn.classList.remove('opacity-60','cursor-not-allowed'); loading=false; loadMoreBtn.disabled=!hasMore; }
    }
    ...
    chk.addEventListener('change', ()=>{ const enabled=chk.checked; localStorage.setItem(LS_KEY, enabled); if(enabled){ startInterval(); fetchActivities({append:false}); } else { clearIntervalRefresh(); } });
    ...
    fetchActivities({append:false, showSkeleton:true}); if(saved) startInterval();
  })();

  /* Captura de token (movido desde HTML para limpiar plantilla) */
  (function(){
    try {
