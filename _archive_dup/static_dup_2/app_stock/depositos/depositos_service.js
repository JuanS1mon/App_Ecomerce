/**
 * JavaScript para la gestión de depositos
...
async function fetchData() {
    try {
        const response = await fetch('/depositos/');
...
function updateTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        cell3.textContent = item.observacion;
...
function editItem(id) {
    fetch(`/depositos/id/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit-descripcion').value = data.descripcion;document.getElementById('edit-codigo').value = data.codigo;document.getElementById('edit-observacion').value = data.observacion;
...
async function updateItem(event) {
    ...
    try {
        const response = await fetch(`/depositos/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/depositos/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.descripcion).toLowerCase().includes(searchTerm) || String(item.codigo).toLowerCase().includes(searchTerm) || String(item.observacion).toLowerCase().includes(searchTerm);
    });
