/**
 * JavaScript para la gestión de articulos_series
...
async function fetchData() {
    try {
        const response = await fetch('/articulos_series/');
...
async function addItem(event) {
    ...
    const formData = {
        // Ya no enviamos el id, será generado automáticamente por la base de datos
...
async function updateItem(event) {
    ...
    try {
        const response = await fetch(`/articulos_series/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/articulos_series/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.serie).toLowerCase().includes(searchTerm);
    });
