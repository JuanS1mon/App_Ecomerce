...
async def _handle_date_conversion(request: ChangeFieldTypeRequest, db: Session, current_user: UserDB):
    ...
    try:
        ...
        with db.get_bind().begin() as conn:
            ...
            result = conn.execute(text(f"""
                SELECT COUNT(*) FROM [{request.table_name}] 
                WHERE [{request.field_name}] IS NOT NULL 
                AND [{request.field_name}_temp] IS NULL
            """)).scalar()
            
            if result > 0:
                # Si hay valores problemáticos, no hacer el cambio
                samples = conn.execute(text(f"""
                    SELECT TOP 5 [{request.field_name}] FROM [{request.table_name}] 
                    WHERE [{request.field_name}] IS NOT NULL
                    AND [{request.field_name}_temp] IS NULL
                """)).fetchall()
                ...
                # Limpiar columna temporal - esto se ejecutará dentro de la misma transacción
                conn.execute(text(f"ALTER TABLE [{request.table_name}] DROP COLUMN [{request.field_name}_temp]"))
                
                # No necesitamos rollback() aquí porque with conn.begin() lo hará automáticamente al lanzar una excepción
                raise ValueError(f"No se puede convertir la columna a DATE porque hay {result} valores que no son fechas válidas.")
            
            # 6. Completar la conversión si todo está bien - ESTO ES CRÍTICO
            # Primero eliminar la columna original
            conn.execute(text(f"ALTER TABLE [{request.table_name}] DROP COLUMN [{request.field_name}]"))
            
            # Luego renombrar la columna temporal
            conn.execute(text(f"EXEC sp_rename '[{request.table_name}].[{request.field_name}_temp]', '{request.field_name}', 'COLUMN'"))
            
            # Verificación adicional para confirmar el cambio
            new_type = conn.execute(text(f"""
                SELECT DATA_TYPE 
                FROM INFORMATION_SCHEMA.COLUMNS 
                WHERE TABLE_NAME = '{request.table_name}' 
                AND COLUMN_NAME = '{request.field_name}'
            ...
            if new_type.upper() != 'DATE':
                raise ValueError(f"La conversión falló. El tipo actual es {new_type} en lugar de DATE.")
