...
async def routes_post_familias(familias: FamiliasCreate, db: Session = Depends(get_db)):
    if familias.codigo is None or familias.descrip is None:
...
@router.get("/id/{codigo}", response_model=FamiliasRead)
async def routes_get_familias_codigo(codigo: int, db: Session = Depends(get_db)):
    try:
        db_familias = get_familias(db, codigo)
        if not db_familias:
...
@router.delete("/id/{codigo}", response_model=FamiliasRead)
async def routes_delete_familias_numero(codigo: int, db: Session = Depends(get_db)):
    try:
        resultado_familias = get_familias(db, codigo)
        if not resultado_familias:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Route: familias no encontrado")
        db_familias = delete_familias(db, codigo)
        return FamiliasRead.model_validate(db_familias)
    except Exception as e:
...
@router.put("/id/{codigo}", response_model=FamiliasRead)
async def routes_update_familias(codigo: int, familias: FamiliasUpdate, db: Session = Depends(get_db)):
    logger.info(f"Actualizando Familias con codigo = {codigo}")
    try:
        familias_data = familias.model_dump()
        db_familias = update_familias(db=db, codigo=codigo, familias_data=familias_data)
        return FamiliasRead.model_validate(db_familias)
    except Exception as e:
