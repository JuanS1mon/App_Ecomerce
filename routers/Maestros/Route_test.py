...
async def routes_post_test(test: TestCreate, db: Session = Depends(get_db)):
    if test.codigo is None or test.nombre is None:
...
@router.get("/id/{codigo}", response_model=TestRead)
async def routes_get_test_codigo(codigo: int, db: Session = Depends(get_db)):
    try:
        db_test = get_test(db, codigo)
        if not db_test:
...
@router.delete("/id/{codigo}", response_model=TestRead)
async def routes_delete_test_numero(codigo: int, db: Session = Depends(get_db)):
    try:
        resultado_test = get_test(db, codigo)
        if not resultado_test:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Route: test no encontrado")
        db_test = delete_test(db, codigo)
        return TestRead.model_validate(db_test)
    except Exception as e:
...
@router.put("/id/{codigo}", response_model=TestRead)
async def routes_update_test(codigo: int, test: TestUpdate, db: Session = Depends(get_db)):
    logger.info(f"Actualizando Test con codigo = {codigo}")
    try:
        test_data = test.model_dump()
        db_test = update_test(db=db, codigo=codigo, test_data=test_data)
        return TestRead.model_validate(db_test)
    except Exception as e:
