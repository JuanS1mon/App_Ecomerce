...
router = APIRouter(
    prefix="/ecommerce/auth",
    tags=["ecommerce-auth"],
)

@router.post("/register")
def register_ecommerce_user_endpoint(
    ...
    try:
        ...
        registered_user = register_ecommerce_user(db, user_dict)
    ...
    try:
        ...
        if not user_email or not user_password:
            ...
            if is_api_request:
                ...
                return RedirectResponse(
                    url="/ecommerce/login?error=Credenciales requeridas",
                    status_code=status.HTTP_303_SEE_OTHER
                )

        # Autenticar usuario
        user = authenticate_ecommerce_user(db, user_email, user_password)
        ...
        if not user:
            ...
            if is_api_request:
                ...
                return RedirectResponse(
                    url="/ecommerce/login?error=Credenciales incorrectas",
                    status_code=status.HTTP_303_SEE_OTHER
                )
...
@router.post("/logout")
def logout_ecommerce_user_endpoint(
    request: Request,
...
@router.get("/me")
def get_current_ecommerce_user_info_endpoint(
    request: Request,
    credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer(auto_error=False)),
    db: Session = Depends(get_db)
):
...
@router.get("/verify")
def verify_ecommerce_token_endpoint(
    credentials: HTTPAuthorizationCredentials = Depends(HTTPBearer(auto_error=False)),
    db: Session = Depends(get_db)
):
    """
...
def get_ecommerce_auth_status(
    ...
    try:
        ...
        # Validar token
        user = get_current_ecommerce_user(token, db)
