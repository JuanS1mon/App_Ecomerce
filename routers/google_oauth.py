    ...
    try:
        ...
        try:
            ...
            return RedirectResponse(
                url=f"/ecommerce/login?error=Error de autenticación con Google",
        ...
        user = create_or_update_google_user(db, validated_user_info)
        ...
        log_google_auth_event(
            "GOOGLE_LOGIN_SUCCESS",
            {
                "email": user.email,
                "user_id": user.id,
                "is_new_user": user.created_at.timestamp() > (user.created_at.timestamp() - 60),
                **client_info
            },
            "INFO"
        )
        ...
        return RedirectResponse(
            url=f"/ecommerce/login?error={e.detail}",
            status_code=status.HTTP_303_SEE_OTHER
        )
    except Exception as e:
        ...
        return RedirectResponse(
            url="/ecommerce/login?error=Error procesando autenticación con Google",
            status_code=status.HTTP_303_SEE_OTHER
        )
