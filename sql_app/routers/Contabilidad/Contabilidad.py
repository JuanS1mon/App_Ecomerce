...
from db.schemas.Contabilidad.asientos import Asiento,AsientoRespuesta,Asiento_respuesta,AsientoRespuestas
from sqlalchemy.orm import Session
from sqlalchemy.exc import SQLAlchemyError
from db.crud.Contabilidad.asientos import get_asiento,gets_asiento,create_asiento,delete_asiento,update_asiento,consulta_Mayores
...
# LA EMPRESA TIENE QUE SER LA DE SISTEMA O LA QUE SE ESTE USANDO EN EL MOMENTO ??????????? : Fue la tomo de sistema.empresadefecto
...
async def post_Asiento(Asiento: Asiento, db: Session = Depends(get_db)):#AsientoCreate recibe datos de asiento
    ...
    if suma_D != suma_H:
        raise HTTPException(status_code=410, detail="El total Debe no coincide con el total Haber")

    try:
        result = create_asiento(db=db, fecha=Asiento.fecha, detalle=Asiento.detalle, tipomovimiento=Asiento.tipomovimiento, asiento_detalle=Asiento.asiento_detalle)
        return result
...
async def get_Asiento(codigo: int, db: Session = Depends(get_db)):
    try:
    ...
    asiento = get_asiento(db, asiento=codigo_asiento)
    if asiento is None:
...
async def post_Asientos(busqueda: dict, db: Session = Depends(get_db)):
    ...
    asientos = gets_asiento(db, busqueda.get('fechadesde'), busqueda.get('fechahasta'), busqueda.get('TipodeMovimiento'), busqueda.get('codigos_asientos'))
    if not asientos:
...
async def delete_Asiento(asiento: int, db: Session = Depends(get_db)):
    ...
    db_Asiento = get_asiento(db, asiento=asiento) 
    print(db_Asiento)
    ...
    delete_asiento(db=db, asiento=asiento)
...
async def update_Asiento(codigo: int, asiento: Asiento, db: Session = Depends(get_db)):
    # Intenta obtener la Asiento con el código proporcionado
    db_Asiento = get_asiento(db, codigo=codigo)
    ...
    updated_Asiento = update_asiento(_db=db, codigo=codigo, asiento=asiento)
    # Devuelve la Asiento que se actualizó
    return updated_Asiento


@router.post("/mayor_cuentas")
async def post_Mayor(busqueda: dict, db: Session = Depends(get_db)):
    try:
        if busqueda.get('cuentas'):
            busqueda['cuentas'] = [int(cuenta) for cuenta in busqueda['cuentas']]
    except ValueError:
        raise HTTPException(status_code=400, detail="Las cuentas deben ser números enteros válidos")
    fecha_desde = busqueda.get('fecha_desde')  # Cambiado aquí
    fecha_hasta = busqueda.get('fecha_hasta')  # Cambiado aquí
    cuenta_desde = busqueda.get('cuenta_desde')  # Cambiado aquí
    cuenta_hasta = busqueda.get('cuenta_hasta')  # Cambiado aquí
    try:
        print (fecha_desde, fecha_hasta, cuenta_desde, cuenta_hasta)
        mayor = consulta_Mayores(db, fecha_desde, fecha_hasta, cuenta_desde, cuenta_hasta)
        if not mayor:
            raise HTTPException(status_code=404, detail="Mayor no encontrado") 
        return mayor
    except ValueError as e:
        raise HTTPException(status_code=400, detail=f"Error de datos Decimal to float: {str(e)}")
    except SQLAlchemyError as e:
        raise HTTPException(status_code=500, detail=f"Error inesperado en la consulta del mayor: {str(e)}")
