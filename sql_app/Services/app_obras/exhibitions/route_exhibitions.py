...
# Rutas HTML
@router.get("/html/test", response_class=HTMLResponse)
async def get_exhibitions_test():
    """Ruta de prueba simple sin dependencias"""
    return HTMLResponse(content="<h1>Test de Exhibitions - Funciona!</h1>")

@router.get("/html/", response_class=HTMLResponse)
async def get_exhibitions_html(request: Request, db: Session = Depends(get_db)):
    try:
        exhibitions = get_all_exhibitions(db)
        return templates.TemplateResponse("app_obras/exhibitions/list.html", {
            "request": request,
...
async def get_create_exhibitions_html(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        return templates.TemplateResponse("app_obras/exhibitions/create.html", {
            "request": request,
            "artworks": artworks,
...
async def get_edit_exhibitions_html(request: Request, id: int, db: Session = Depends(get_db)):
    try:
        ...
        return templates.TemplateResponse("app_obras/exhibitions/edit.html", {
            "request": request,
