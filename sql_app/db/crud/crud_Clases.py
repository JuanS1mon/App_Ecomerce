...
from ..schemas.Clases import ClasesCreate

def create(db: Session, Clases: ClasesCreate):
    try:
        sql = text("INSERT INTO Clases(Codigo, Descripcion, Transmitido) VALUES(:Codigo, :Descripcion, :Transmitido)")
        db.execute(sql.params(Clases=Clases))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Clases WHERE Codigo = :Codigo"), {"Codigo": Clases.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Clases")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Clases WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        Clases = result.fetchall()
        if Clases is None:
            raise HTTPException(status_code=404, detail="Clases no encontrado")
        return Clases
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Clases")
