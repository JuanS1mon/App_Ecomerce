...
from ..schemas.TiposCbtes import TiposcbtesCreate

def create(db: Session, TiposCbtes: TiposcbtesCreate):
    try:
        sql = text("INSERT INTO TiposCbtes(Codigo, Descripcion, Signo, IVA) VALUES(:Codigo, :Descripcion, :Signo, :IVA)")
        db.execute(sql.params(TiposCbtes=TiposCbtes))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Signo, IVA FROM TiposCbtes WHERE Codigo = :Codigo"), {"Codigo": TiposCbtes.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el TiposCbtes")

def get(db: Session, Codigo: NVARCHAR):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Signo, IVA FROM TiposCbtes WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        TiposCbtes = result.fetchall()
        if TiposCbtes is None:
            raise HTTPException(status_code=404, detail="Tiposcbtes no encontrado")
        return TiposCbtes
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el TiposCbtes")
