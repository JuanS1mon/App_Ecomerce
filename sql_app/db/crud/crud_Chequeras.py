...
from ..schemas.Chequeras import ChequerasCreate

def create(db: Session, Chequeras: ChequerasCreate):
    try:
        sql = text("INSERT INTO Chequeras(Empresa, Numero, Banco, Desde, Hasta, FechaChequera) VALUES(:Empresa, :Numero, :Banco, :Desde, :Hasta, :FechaChequera)")
        db.execute(sql.params(Chequeras=Chequeras))
        db.commit()
        result = db.execute(text("SELECT Empresa, Numero, Banco, Desde, Hasta, FechaChequera FROM Chequeras WHERE Empresa = :Empresa"), {"Empresa": Chequeras.Empresa})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Chequeras")

def get(db: Session, Empresa: INTEGER):
    try:
        result = db.execute(text("SELECT Empresa, Numero, Banco, Desde, Hasta, FechaChequera FROM Chequeras WHERE Empresa = :Empresa"), {"Empresa": Empresa})
        Chequeras = result.fetchall()
        if Chequeras is None:
            raise HTTPException(status_code=404, detail="Chequeras no encontrado")
        return Chequeras
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Chequeras")
