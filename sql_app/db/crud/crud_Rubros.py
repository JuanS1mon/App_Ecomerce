...
from ..schemas.Rubros import RubrosCreate

def create(db: Session, Rubros: RubrosCreate):
    try:
        sql = text("INSERT INTO Rubros(Codigo, Descripcion, Transmitido) VALUES(:Codigo, :Descripcion, :Transmitido)")
        db.execute(sql.params(Rubros=Rubros))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Rubros WHERE Codigo = :Codigo"), {"Codigo": Rubros.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Rubros")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Rubros WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        Rubros = result.fetchall()
        if Rubros is None:
            raise HTTPException(status_code=404, detail="Rubros no encontrado")
        return Rubros
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Rubros")
