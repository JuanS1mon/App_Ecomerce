...
from ..schemas.Provincias import ProvinciasCreate

def create(db: Session, Provincias: ProvinciasCreate):
    try:
        sql = text("INSERT INTO Provincias(Codigo, Descripcion, Transmitido) VALUES(:Codigo, :Descripcion, :Transmitido)")
        db.execute(sql.params(Provincias=Provincias))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Provincias WHERE Codigo = :Codigo"), {"Codigo": Provincias.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Provincias")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Provincias WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        Provincias = result.fetchall()
        if Provincias is None:
            raise HTTPException(status_code=404, detail="Provincias no encontrado")
        return Provincias
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Provincias")
