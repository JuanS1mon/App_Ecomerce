...
from ..schemas.BancosCh3 import Bancosch3Create

def create(db: Session, BancosCh3: Bancosch3Create):
    try:
        sql = text("INSERT INTO BancosCh3(Codigo, Descripcion) VALUES(:Codigo, :Descripcion)")
        db.execute(sql.params(BancosCh3=BancosCh3))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion FROM BancosCh3 WHERE Codigo = :Codigo"), {"Codigo": BancosCh3.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el BancosCh3")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion FROM BancosCh3 WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        BancosCh3 = result.fetchall()
        if BancosCh3 is None:
            raise HTTPException(status_code=404, detail="Bancosch3 no encontrado")
        return BancosCh3
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el BancosCh3")
