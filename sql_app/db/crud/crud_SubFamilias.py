...
from ..schemas.SubFamilias import SubfamiliasCreate

def create(db: Session, SubFamilias: SubfamiliasCreate):
    try:
        sql = text("INSERT INTO SubFamilias(Codigo, Descripcion, Familia, Transmitido) VALUES(:Codigo, :Descripcion, :Familia, :Transmitido)")
        db.execute(sql.params(SubFamilias=SubFamilias))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Familia, Transmitido FROM SubFamilias WHERE Codigo = :Codigo"), {"Codigo": SubFamilias.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el SubFamilias")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Familia, Transmitido FROM SubFamilias WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        SubFamilias = result.fetchall()
        if SubFamilias is None:
            raise HTTPException(status_code=404, detail="Subfamilias no encontrado")
        return SubFamilias
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el SubFamilias")
