...
from ..schemas.Encabezado import EncabezadoCreate

def create(db: Session, Encabezado: EncabezadoCreate):
    try:
        sql = text("INSERT INTO Encabezado(Linea, Texto, Doble, Sucursal) VALUES(:Linea, :Texto, :Doble, :Sucursal)")
        db.execute(sql.params(Encabezado=Encabezado))
        db.commit()
        result = db.execute(text("SELECT Linea, Texto, Doble, Sucursal FROM Encabezado WHERE Linea = :Linea"), {"Linea": Encabezado.Linea})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Encabezado")

def get(db: Session, Linea: INTEGER):
    try:
        result = db.execute(text("SELECT Linea, Texto, Doble, Sucursal FROM Encabezado WHERE Linea = :Linea"), {"Linea": Linea})
        Encabezado = result.fetchall()
        if Encabezado is None:
            raise HTTPException(status_code=404, detail="Encabezado no encontrado")
        return Encabezado
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Encabezado")
