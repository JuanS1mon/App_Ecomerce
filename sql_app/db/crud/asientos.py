...
def create(db: Session, fecha: datetime, detalle: str, tipomovimiento: int, asiento_detalle):
    try:
...
def get(db: Session, asiento: int):
    try:
...
def gets(db: Session, fechadesde: str, fechahasta: str, TipodeMovimiento: List[int], codigos_asientos: List[int]):
    try:
...
def update(db: Session, asiento: int):
    try:
        # Actualizar Asiento
...
def get_saldos(db: Session, fecha_desde: str, fecha_hasta: str, cuenta_desde: int, cuenta_hasta: int):
    try:
        sql = """
        WITH CTE_Saldos AS (
            SELECT
