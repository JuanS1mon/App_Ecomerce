...
from ..schemas.Sectores import SectoresCreate

def create(db: Session, Sectores: SectoresCreate):
    try:
        sql = text("INSERT INTO Sectores(Codigo, Descripcion, Transmitido) VALUES(:Codigo, :Descripcion, :Transmitido)")
        db.execute(sql.params(Sectores=Sectores))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Sectores WHERE Codigo = :Codigo"), {"Codigo": Sectores.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Sectores")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido FROM Sectores WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        Sectores = result.fetchall()
        if Sectores is None:
            raise HTTPException(status_code=404, detail="Sectores no encontrado")
        return Sectores
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Sectores")
