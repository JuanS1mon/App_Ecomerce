...
from ..schemas.Zonas import ZonasCreate

def create(db: Session, Zonas: ZonasCreate):
    try:
        sql = text("INSERT INTO Zonas(Codigo, Descripcion, Transmitido, ImporteTransporte) VALUES(:Codigo, :Descripcion, :Transmitido, :ImporteTransporte)")
        db.execute(sql.params(Zonas=Zonas))
        db.commit()
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido, ImporteTransporte FROM Zonas WHERE Codigo = :Codigo"), {"Codigo": Zonas.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Zonas")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, Descripcion, Transmitido, ImporteTransporte FROM Zonas WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        Zonas = result.fetchall()
        if Zonas is None:
            raise HTTPException(status_code=404, detail="Zonas no encontrado")
        return Zonas
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Zonas")
