...
from ..schemas.Articulos import ArticulosCreate

def create(db: Session, Articulos: ArticulosCreate):
    try:
        sql = text("INSERT INTO Articulos(Codigo, EAN, Descripcion, DescripcionCorta, PrecioCosto, PrecioVenta, Margen, IVA, EsCombo, Proveedor, Departamento, Marca, Familia, UxB, HabilCajas, HabilEstad, HabilStock, HabilOC, HabilBalanzas, BalDiasVto, BalCodigo, Tipo, CodigoEnvase, FechaModificacion, FechaAlta, HoraModificacion, Transmitido, Deposito, PresentacionCantidad, PresentacionUnidad, Ruteo, Alfa, Peso, StockMinimo, CxB, DescuentoXCaja, ImpInterno, ArMargenTeorico, CuentaContable, SubFamilia, DiasAlVto, MargenIIBB, MargenOtrosImp, ModeloEtiqueta, CantidadEtiquetas, CxP, CxPP, GeneraRemitoComponentes, Serializado, Sector) VALUES(:Codigo, :EAN, :Descripcion, :DescripcionCorta, :PrecioCosto, :PrecioVenta, :Margen, :IVA, :EsCombo, :Proveedor, :Departamento, :Marca, :Familia, :UxB, :HabilCajas, :HabilEstad, :HabilStock, :HabilOC, :HabilBalanzas, :BalDiasVto, :BalCodigo, :Tipo, :CodigoEnvase, :FechaModificacion, :FechaAlta, :HoraModificacion, :Transmitido, :Deposito, :PresentacionCantidad, :PresentacionUnidad, :Ruteo, :Alfa, :Peso, :StockMinimo, :CxB, :DescuentoXCaja, :ImpInterno, :ArMargenTeorico, :CuentaContable, :SubFamilia, :DiasAlVto, :MargenIIBB, :MargenOtrosImp, :ModeloEtiqueta, :CantidadEtiquetas, :CxP, :CxPP, :GeneraRemitoComponentes, :Serializado, :Sector)")
        db.execute(sql.params(Articulos=Articulos))
        db.commit()
        result = db.execute(text("SELECT Codigo, EAN, Descripcion, DescripcionCorta, PrecioCosto, PrecioVenta, Margen, IVA, EsCombo, Proveedor, Departamento, Marca, Familia, UxB, HabilCajas, HabilEstad, HabilStock, HabilOC, HabilBalanzas, BalDiasVto, BalCodigo, Tipo, CodigoEnvase, FechaModificacion, FechaAlta, HoraModificacion, Transmitido, Deposito, PresentacionCantidad, PresentacionUnidad, Ruteo, Alfa, Peso, StockMinimo, CxB, DescuentoXCaja, ImpInterno, ArMargenTeorico, CuentaContable, SubFamilia, DiasAlVto, MargenIIBB, MargenOtrosImp, ModeloEtiqueta, CantidadEtiquetas, CxP, CxPP, GeneraRemitoComponentes, Serializado, Sector FROM Articulos WHERE Codigo = :Codigo"), {"Codigo": Articulos.Codigo})
        return result
    except SQLAlchemyError as e:
        db.rollback()
        raise HTTPException(status_code=403, detail="No se pudo crear el Articulos")

def get(db: Session, Codigo: INTEGER):
    try:
        result = db.execute(text("SELECT Codigo, EAN, Descripcion, DescripcionCorta, PrecioCosto, PrecioVenta, Margen, IVA, EsCombo, Proveedor, Departamento, Marca, Familia, UxB, HabilCajas, HabilEstad, HabilStock, HabilOC, HabilBalanzas, BalDiasVto, BalCodigo, Tipo, CodigoEnvase, FechaModificacion, FechaAlta, HoraModificacion, Transmitido, Deposito, PresentacionCantidad, PresentacionUnidad, Ruteo, Alfa, Peso, StockMinimo, CxB, DescuentoXCaja, ImpInterno, ArMargenTeorico, CuentaContable, SubFamilia, DiasAlVto, MargenIIBB, MargenOtrosImp, ModeloEtiqueta, CantidadEtiquetas, CxP, CxPP, GeneraRemitoComponentes, Serializado, Sector FROM Articulos WHERE Codigo = :Codigo"), {"Codigo": Codigo})
        Articulos = result.fetchall()
        if Articulos is None:
            raise HTTPException(status_code=404, detail="Articulos no encontrado")
        return Articulos
    except SQLAlchemyError:
        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo obtener el Articulos")
