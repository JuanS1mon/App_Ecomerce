...
from .file_upload import process_and_save_image
...
async def get_artworks_html(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        return templates.TemplateResponse("app_obras/artworks/list.html", {
            "request": request,
            "artworks": artworks
        })
    except Exception as e:
...
async def get_create_artworks_html(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("app_obras/artworks/create.html", {"request": request})
...
async def get_edit_artworks_html(request: Request, id: int, db: Session = Depends(get_db)):
    try:
        ...
        if not artwork:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Artwork no encontrado")
        return templates.TemplateResponse("app_obras/artworks/edit.html", {
            "request": request,
            "artwork": artwork
        })
    except HTTPException:
...
async def upload_artwork_image(file: UploadFile = File(...)):
    """Endpoint para subir imágenes de obras de arte"""
    try:
        result = await process_and_save_image(file)
        return {
            "success": True,
            "message": "Imagen subida exitosamente",
            "url": result["thumbnail_url"],  # Devolver URL del thumbnail para mostrar en listado
            "data": result
        }
    except HTTPException:
        raise
