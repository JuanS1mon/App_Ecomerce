...
def user_pass(db: Session, username: str, password: str):
    ...
    El parámetro password se mantiene por compatibilidad pero no se usa en la validación.
    La validación real de la contraseña se hace en authenticate_user.
    """
    try:
        result = db.execute(text("SELECT clave FROM Usuarios WHERE usuario = :usuario"), {"usuario": username})
        row = result.fetchone()
        if not row:
            logger.warning(f"Usuario no encontrado en user_pass: {username}")
            return None
        hashed_password = row[0]  # Extrae la contraseña encriptada del objeto Row
        if not hashed_password:
            logger.warning(f"Usuario {username} no tiene contraseña configurada")
            return None
        return hashed_password  # Devolver solo el hash, no un diccionario
    except Exception as e:
        logger.error(f"Error en user_pass para usuario {username}: {str(e)}")
        return None
...
def update_usuario_activate(db: Session, usuario: str):
    try:
        ...
        db.commit()
        return [{"codigo": row[0], "usuario": row[1], "nombre": row[2], "email": row[3]} for row in rows]  # Return updated data
    except SQLAlchemyError:        db.rollback()
        raise HTTPException(status_code=400, detail="No se pudo actualizar el usuario")
...
def get_user_from_db(db: Session, username: str):
    ...
    try:
        ...
        user_dict = {
            ...
            "password": user_result.clave,  # Agregar password para compatibilidad con security.py
        }
