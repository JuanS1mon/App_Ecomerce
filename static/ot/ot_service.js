/**
 * JavaScript para la gestión de ot
...
function editItem(id) {
    fetch(`/ot/id/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit-id_trabajo').value = data.id_trabajo;document.getElementById('edit-area').value = data.area;document.getElementById('edit-personal').value = data.personal;document.getElementById('edit-tiempo_estimado').value = data.tiempo_estimado;
...
async function updateItem(event) {
    ...
    const updatedData = {
        id_trabajo: document.getElementById('edit-id_trabajo').value,
        area: document.getElementById('edit-area').value,
        personal: document.getElementById('edit-personal').value,
        tiempo_estimado: document.getElementById('edit-tiempo_estimado').value
    };
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/ot/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.id_trabajo).toLowerCase().includes(searchTerm) || String(item.area).toLowerCase().includes(searchTerm) || String(item.personal).toLowerCase().includes(searchTerm) || String(item.tiempo_estimado).toLowerCase().includes(searchTerm);
    });
