 ...
 */
 async function fetchDepositos() {
    try {
        const response = await fetch('/stock/depositos');
        if (!response.ok) throw new Error("Error al cargar depósitos");

        const data = await response.json();
        const select = document.getElementById('id_deposito');
const destinoSelect = document.getElementById('id_deposito_destino');

select.innerHTML = '<option value="">Seleccione un depósito</option>';
destinoSelect.innerHTML = '<option value="">Seleccione un depósito</option>';

data.forEach(dep => {
    const optionOrigen = document.createElement('option');
    optionOrigen.value = dep.id;
    optionOrigen.textContent = dep.descripcion;
    select.appendChild(optionOrigen);

    const optionDestino = document.createElement('option');
    optionDestino.value = dep.id;
    optionDestino.textContent = dep.descripcion;
    destinoSelect.appendChild(optionDestino);
        ...
        showToast("Error al cargar la lista de depósitos", 'error');
    }
}
...
document.getElementById('id_deposito').addEventListener('change', async function () {
    const idDeposito = this.value;
    if (!idDeposito) return;

    try {
        const response = await fetch(`/stock/stock_actual/${idDeposito}`);
        if (!response.ok) throw new Error("No se pudo obtener el stock");

        const data = await response.json();
        document.getElementById('cant_disponible').value = data.cant_disponible || 0;
        document.getElementById('cant_preparado').value = data.cant_preparado || 0;
    } catch (error) {
        console.error("Error al obtener stock:", error);
        showToast("Error al cargar stock actual del depósito", 'error');
    }
});
...
async function addItem(event) {
    ...
    const formData = {
        ...
        id_deposito_destino: parseInt(document.getElementById('id_deposito_destino').value) || 0,
...
async function updateItem(event) {
    ...
    try {
        const response = await fetch(`/stock/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/stock/id/${id}`, { method: 'DELETE' });
