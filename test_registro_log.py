...
log_filename = os.path.join(log_dir, f"registro_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.DEBUG,
...
datos_usuario = {
    "nombre": "Usuario Prueba Log",
    "usuario": "usuario_test_log",
    "clave": "Contraseña123!",
...
def probar_registro_con_log():
    ...
    try:
        ...
        respuesta_file = os.path.join(log_dir, f"respuesta_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
