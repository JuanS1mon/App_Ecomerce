/**
 * JavaScript para la gestión de carrito_items
...
function updateTable(data) {
    ...
    data.forEach((item, index) => {
        ...
        cell4.textContent = item.precio_unitario;
...
async function addItem(event) {
    ...
    const formData = {
        id_carrito: parseInt(document.getElementById('id_carrito').value),
    ...
    try {
        const response = await fetch('/ecomerce/carrito_items/', {
            method: 'POST',
...
function editItem(id) {
    fetch(`/ecomerce/carrito_items/id/${id}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            document.getElementById('edit-id_carrito').value = data.id_carrito;document.getElementById('edit-id_producto').value = data.id_producto;document.getElementById('edit-cantidad').value = data.cantidad;document.getElementById('edit-precio_unitario').value = data.precio_unitario;
...
async function updateItem(event) {
    ...
    const updatedData = {
        id_carrito: parseInt(document.getElementById('edit-id_carrito').value) || 0,
        ...
        precio_unitario: parseInt(document.getElementById('edit-precio_unitario').value) || 0
    };
    
    try {
        const response = await fetch(`/ecomerce/carrito_items/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/ecomerce/carrito_items/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.id_carrito).toLowerCase().includes(searchTerm) || String(item.id_producto).toLowerCase().includes(searchTerm) || String(item.cantidad).toLowerCase().includes(searchTerm) || String(item.precio_unitario).toLowerCase().includes(searchTerm);
    });
