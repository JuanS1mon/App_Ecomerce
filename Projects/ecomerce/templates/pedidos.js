...
async function addItem(event) {
    ...
    const formData = {
        ...
        fecha_pedido: document.getElementById('fecha_pedido').value || null,
        total: parseInt(document.getElementById('total').value),
        estado: document.getElementById('estado').value || null
    };
    
    try {
        const response = await fetch('/ecomerce/pedidos/', {
            method: 'POST',
...
function editItem(id) {
    fetch(`/ecomerce/pedidos/id/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit-id_usuario').value = data.id_usuario;document.getElementById('edit-fecha_pedido').value = data.fecha_pedido;document.getElementById('edit-total').value = data.total;document.getElementById('edit-estado').value = data.estado;
...
async function updateItem(event) {
    ...
    const updatedData = {
        id_usuario: parseInt(document.getElementById('edit-id_usuario').value) || 0,
        fecha_pedido: document.getElementById('edit-fecha_pedido').value,
        total: parseInt(document.getElementById('edit-total').value) || 0,
        estado: document.getElementById('edit-estado').value
    };
    
    try {
        const response = await fetch(`/ecomerce/pedidos/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/ecomerce/pedidos/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.id_usuario).toLowerCase().includes(searchTerm) || String(item.fecha_pedido).toLowerCase().includes(searchTerm) || String(item.total).toLowerCase().includes(searchTerm) || String(item.estado).toLowerCase().includes(searchTerm);
    });
