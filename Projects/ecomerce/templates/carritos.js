/**
 * JavaScript para la gestión de carritos
...
async function addItem(event) {
    ...
    try {
        const response = await fetch('/ecomerce/carritos/', {
            method: 'POST',
...
function editItem(id) {
    fetch(`/ecomerce/carritos/id/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit-id_usuario').value = data.id_usuario;document.getElementById('edit-estado').value = data.estado;document.getElementById('edit-created_at').value = data.created_at;
...
async function updateItem(event) {
    ...
    try {
        const response = await fetch(`/ecomerce/carritos/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/ecomerce/carritos/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.id_usuario).toLowerCase().includes(searchTerm) || String(item.estado).toLowerCase().includes(searchTerm) || String(item.created_at).toLowerCase().includes(searchTerm);
    });
