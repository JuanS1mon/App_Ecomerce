/**
 * JavaScript para la gestión de stock
...
async function addItem(event) {
    ...
    const formData = {
        ...
        cantidad_reservada: parseInt(document.getElementById('cantidad_reservada').value),
        ubicacion: document.getElementById('ubicacion').value || null,
        updated_at: document.getElementById('updated_at').value || null
    };
    
    try {
        const response = await fetch('/ecomerce/stock/', {
            method: 'POST',
...
function editItem(id) {
    fetch(`/ecomerce/stock/id/${id}`)
        ...
        .then(data => {
            ...
            document.getElementById('edit-id_producto').value = data.id_producto;document.getElementById('edit-cantidad_disponible').value = data.cantidad_disponible;document.getElementById('edit-cantidad_reservada').value = data.cantidad_reservada;document.getElementById('edit-ubicacion').value = data.ubicacion;document.getElementById('edit-updated_at').value = data.updated_at;
...
async function updateItem(event) {
    ...
    const updatedData = {
        id_producto: parseInt(document.getElementById('edit-id_producto').value) || 0,
        cantidad_disponible: parseInt(document.getElementById('edit-cantidad_disponible').value) || 0,
        cantidad_reservada: parseInt(document.getElementById('edit-cantidad_reservada').value) || 0,
        ubicacion: document.getElementById('edit-ubicacion').value,
        updated_at: document.getElementById('edit-updated_at').value
    };
    
    try {
        const response = await fetch(`/ecomerce/stock/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/ecomerce/stock/id/${id}`, { method: 'DELETE' });
...
function filterTable() {
    ...
    const filteredData = allData.filter(item => {
        return String(item.id).toLowerCase().includes(searchTerm) || String(item.id_producto).toLowerCase().includes(searchTerm) || String(item.cantidad_disponible).toLowerCase().includes(searchTerm) || String(item.cantidad_reservada).toLowerCase().includes(searchTerm) || String(item.ubicacion).toLowerCase().includes(searchTerm) || String(item.updated_at).toLowerCase().includes(searchTerm);
    });
