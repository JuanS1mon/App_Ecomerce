...
function updateTable(data) {
    ...
    data.forEach((item) => {
        ...
        const imageUrl = item.imagen_url || '/static/img/placeholder-product.png';
        const price = item.precio ? `$${item.precio.toLocaleString()}` : 'Precio no disponible';
        
        card.innerHTML = `
            <div class="relative overflow-hidden">
                <img src="${imageUrl}" alt="${item.nombre}" class="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='/static/img/placeholder-product.png'">
...
async function addItem(event) {
    ...
    try {
        const response = await fetch('/ecomerce/productos/', {
            method: 'POST',
...
function editItem(id) {
    fetch(`/ecomerce/productos/id/${id}`)
        .then(response => {
            if (!response.ok) {
...
async function updateItem(event) {
    ...
    try {
        const response = await fetch(`/ecomerce/productos/id/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function confirmDelete() {
    ...
    try {
        const response = await fetch(`/ecomerce/productos/id/${id}`, { method: 'DELETE' });
